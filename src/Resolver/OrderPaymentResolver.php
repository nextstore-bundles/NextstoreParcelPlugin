...
namespace Nextstore\SyliusParcelPlugin\Resolver;

use Alphalabs\SyliusParcelPlugin\Entity\Parcel\Parcel;
...
class OrderPaymentResolver
{
    ...
    public function resolve(object $object): void
    {
        if ($object instanceof NextstorePaymentInterface) {
            if ($object->getParcel() instanceof Parcel) {
                return;
            }
    ...
    private function getTargetTransition(OrderInterface $order): ?string
    {
        ...
        foreach ($refundedPayments as $payment) {
            $refundedPaymentTotal += $payment->getAmountMnt();
        }
        ...
        foreach ($completedPayments as $payment) {
            $completedPaymentTotal += $payment->getAmountMnt();
        }
        ...
        foreach ($authorizedPayments as $payment) {
            $authorizedPaymentTotal += $payment->getAmountMnt();
        }
        ...
        foreach ($processingPayments as $payment) {
            $processingPaymentTotal += $payment->getAmountMnt();
        }
